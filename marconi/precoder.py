...
class PrecoderSDP2(Precoder):
    ...
    def solve_cvxpy(self, chan, recv, weights, lvl, tol=1e-4, queue=None):
        # pylint: disable=R0914
        ...
        recv = cvxopt.matrix(recv)

        ropt_re = cvx.Variable(n_rx*n_ue, n_sk*n_ue)
        ropt_im = cvx.Variable(n_rx*n_ue, n_sk*n_ue)

        scov_re = cvx.Variable(n_sk*n_ue, n_sk*n_ue)
        scov_im = cvx.Variable(n_sk*n_ue, n_sk*n_ue)

        scomp_re = cvx.Variable(n_sk*n_ue, n_sk*n_ue)
        scomp_im = cvx.Variable(n_sk*n_ue, n_sk*n_ue)

        eye_sk = cvxopt.matrix(np.eye(n_sk*n_ue))
        z_sk = cvxopt.matrix(np.zeros((n_sk*n_ue, n_sk*n_ue)))

        wsqrt = cvxopt.matrix(np.linalg.cholesky(weights))
        winv = cvxopt.matrix(np.linalg.pinv(weights))

        # Objective
        objective = cvx.trace(scomp_re) / self.noise_pwr
        objective += cvx.sum_squares(ropt_re*wsqrt.H.real() -
                                     ropt_im*wsqrt.H.imag())
        objective += cvx.sum_squares(ropt_re*wsqrt.H.imag() +
                                     ropt_im*wsqrt.H.real())

        prob = cvx.Problem(cvx.Minimize(objective))

        # Postive semidefinite constraints
        A = cvx.vstack(
            cvx.hstack(scomp_re, eye_sk, -scomp_im, z_sk),
            cvx.hstack(eye_sk, scov_re + winv.real(),
                       z_sk, -(scov_im + winv.imag())),
            cvx.hstack(scomp_im, z_sk, scomp_re, eye_sk),
            cvx.hstack(z_sk, scov_im + winv.imag(),
                       eye_sk, scov_re + winv.real()))

        prob.constraints.append(A == cvx.Semidef(A.shape.rows))

        # prob.constraints.append(cvx.vstack(cvx.hstack(scomp_re, -scomp_im),
        #                                    cvx.hstack(scomp_im, scomp_re))
        #                         == cvx.Semidef(2*n_sk*n_ue))

        # prob.constraints.append(cvx.vstack(cvx.hstack(scov_re, -scov_im),
        #                                    cvx.hstack(scov_im, scov_re))
        #                         == cvx.Semidef(2*n_sk*n_ue))

        # Skew-symmetricity
        prob.constraints.append(scov_im + scov_im.T == 0)
        prob.constraints.append(scomp_im + scomp_im.T == 0)

        cpnt = cvxopt.matrix(np.dot(np.dot(recv.H, cov), recv))

        reff = cvxopt.matrix(np.dot(recv.H, cov))

        la_re = cpnt.real() + \
            reff.real() * (ropt_re - recv.real()) - \
            reff.imag() * (ropt_im - recv.imag()) + \
            (ropt_re.T - recv.real().T) * reff.real().T - \
            (ropt_im.T - recv.imag().T) * reff.imag().T

        prob.constraints.append(la_re == scov_re)

        la_im = cpnt.imag() + \
            reff.real() * (ropt_im - recv.imag()) + \
            reff.imag() * (ropt_re - recv.real()) - \
            (ropt_re.T - recv.real().T) * reff.imag().T - \
            (ropt_im.T - recv.imag().T) * reff.real().T

        prob.constraints.append(la_im == scov_im)
        ...
        [prob.constraints.append(ropt_re[_i, _j] == 0) for (_i, _j) in zind]
        [prob.constraints.append(ropt_im[_i, _j] == 0) for (_i, _j) in zind]
    ...
    def search(self, chan, recv, weights, method="bisection", step=0.5):
        ...
        while err > self.precision:
            ...
            ropt = self.solve_cvxpy(chan, recv, weights, self.lvl, tol)
            #p_sandbox.start()
            #p_sandbox.join()
            #ropt = queue.get()
